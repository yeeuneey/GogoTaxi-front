...
const SEAT_TEMPLATE = import.meta.env.VITE_ROOM_SEAT_PATH?.trim() || '/api/rooms/:id/seat'
...
export async function changeSeatFromApi(roomId: string, seatNumber: number) {
  ...
  const url = buildSeatUrl(roomId, seatNumber)
...
function buildSeatUrl(roomId: string, seatNumber: number) {
  let template = SEAT_TEMPLATE

  const replaceTokens = (tokens: string[], value: string) => {
    let replaced = false
  ...
  const idTokens = [':id', '{id}', ':roomId', '{roomId}']
  const seatTokens = [':seatNumber', '{seatNumber}', ':seat', '{seat}']

  const idReplaced = replaceTokens(idTokens, roomId)
  if (!idReplaced) {
    template = `${template.replace(/\/$/, '')}/${roomId}`
  }

  replaceTokens(seatTokens, String(seatNumber))
